...
def execute_worker(pipe,args):
    ...
    import torch,psutil
    from src.retrieval.case_internal import LocalCaseCrossEncoder
...
def main():
    ...
    write(args.output/'INPUT_SEAL.json',{'candidate_inputs_sha256':sha(args.inputs),'chunks_sha256':sha(args.data/'chunks/cases.jsonl'),
        ...
        'torch_threads':8,'cross_encoder_code_sha256':sha(Path(__file__).resolve().parents[1]/'src/retrieval/case_internal.py'),
