...
def main():
    ...
    local_embedding=json.loads((args.root/'02_models/KURE_LOCAL_COPY.json').read_text('utf-8'))
    assert local_embedding['revision']==profile['model_revision']
    profile['embedding_local']={'path':str(args.root/'02_models/kure-v1'),'files':local_embedding['files']}
    if profile['policy']['rerank']=='cross_encoder':
        ...
        metadata=json.loads((args.root/'02_models'/meta).read_text('utf-8'))
        model_path=args.root/'02_models'/name
