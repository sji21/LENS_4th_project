...
대화 기능(`CHAT_CONVERSATION_ENABLED=true`)에서는 첨부한 등기·등본·등기부등본을
해당 대화에서 상담하는 건물의 문서로 연결합니다. “이집 계약해도 괜찮을까?”는
문서의 위험요소와 계약 전 확인사항을 묻는 요청으로 처리합니다. 후속 질문에서도
문서를 유지하되 삭제하거나 다른 상담 대상으로 전환하면 이전 문서를 제외합니다.
계약서는 첨부만으로 체결 여부를 추정하지 않고 사용자가 밝힌 계약 전·후 상태를
반영합니다. 구현·검증 한계는 [PATCH-063](docs/patch063-document-consultation.md)를 참고하세요.
