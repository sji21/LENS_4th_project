...
| 김정재 | 팀원 | 추가 기능 기획 및 구현, LLM 엔지니어링 |
| 송지섭 | 팀원 | 법령 데이터·리트리버 담당. 법령 데이터 수집·구조화 및 DB 구축, 하이브리드 검색 개선·성능 평가, 법령 인용 검증 보완 |
| 윤지환 | 팀원 | 판례 DB 개발 및 데이터 확장, API 기반 대량 판례 수집·정제, 벡터 DB·인덱스 구축, 검색 결과·성능 확인 |
| 신진호 | 팀원 | Streamlit → Django 웹 전환, 사용자 의도 판단, 대화형 챗봇(대화 연결·요약·후속질문) 개선 |
...
| 민법 | 3 | 일반 법령 자리와 별도. 질문에서 감지한 민법 조문을 먼저 두고 검색 후보로 채움 |
| 기관 안내·공식 서식 | 2 | 질문이 해당 절차·서식 주제일 때만 (0~2건) |
| 판례 | 2 | 판례를 요청했거나 구체적 사실의 법적 효과를 묻는 질문, 또는 법령·안내 근거가 부족할 때만 |
...
단위·통합·회귀 검사입니다. [4장의 리트리버 평가](#41-리트리버-3차에서-4차로)처럼 고정 질문에서
...
- 판례 재순위: 필수 판례가 후보 안에 있어도 상위 2~5건에 들지 못하는 문제 개선
- 법령·민법 근거 손실 보완: 민법 복수 근거와 갱신 조문 동반 선택, 질문이 요구할 때의 위임 시행령 전달,
  필요 없는 질문의 민법 후보 제외
- 개선에 사용하지 않은 새 질문을 평가 전에 봉인한 검색·답변 독립 평가
- Qwen3.8-27B 답변 생성 평가(HO30·DEV100-v2) 결과 정리. 검색 예산(법령 3·판례 2)은 8B 기준으로 정했고 27B
  전환 뒤에도 비교를 위해 유지 중이므로 공개 개발셋에서 확대 여부를 따로 측정
- 대화형 상담 모드의 전체 답변 품질·지연 검증 후 기본 경로 전환 여부 결정
- 과거 PATCH-042 평가 기록과 현재 테스트의 불일치 정리, 기관 안내·공식 서식 확대와 정량 평가

패치별 상태와 세부 조건은 [`LIST.md`](LIST.md)와 [리트리버 평가 기록](docs/patch058-retrieval-coverage.md)을
따릅니다.
