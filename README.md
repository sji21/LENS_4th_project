...
- **PATCH-060:** OS별 코드 줄바꿈 통일, CPU 간 벡터 오차 검증, 본문·메타데이터 타입·임베딩 출처의 정확한 검증, 새 환경의 설치 제약 파일 생성 처리를 완료했습니다. 관련 테스트 31개와 r2 배포본 Windows `verify`를 확인했습니다. 초기 배포의 Apple Silicon 성공 기록과 r2의 검증 범위는 구분하며, r2 Mac·Linux 재실측은 이 완료 기록에 포함하지 않습니다.

팀원에게는 **`LENS-MySQL-search-patch060-r2-20260921.zip`과 해당 검색 코드 버전**을 함께 전달합니다. 구형 `shared-v3`, `patch059-20260921`, 1차 `patch060-20260921` 배포본과 혼용하지 않습니다. r2 release ID는 `5b1a63a7b004f63ad2dae018d53b9e4e0cdd120a61d623ec3270174c53622703`입니다.

ZIP에는 검색 release와 KURE 모델이 포함됩니다. 프로젝트 루트에 풀고 가상환경에서 `requirements` → 의존성 설치 → `verify` → `model` → `activate` → 앱 재시작 순서로 적용합니다. 팀원이 MySQL에 재적재하거나 개별 임베딩을 만들 필요는 없습니다. `.env`와 `active.json`은 각 PC에서 설정합니다. 상세 명령과 지원 환경은 [MySQL 검색 설치 안내](docs/mysql-search.md)를 따릅니다.
