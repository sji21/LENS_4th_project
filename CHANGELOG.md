...
- PATCH-027 adds conservative same-edition Civil Act reference parsing and revalidation, clause-level notice gating, and public/private rental context separation. Freeze `context_both + context_reference` before corrected measurements and add a development service adapter for fresh ranking/body/source and case/guide checks on all 235 inputs, with measured embedding calls. The earlier 43/75 and 42/75 preview was rank recombination, not completed live integration. Corrected measurement is pending; operating retrieval/data remain unchanged. (commit: pending)
