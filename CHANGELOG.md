...
- PATCH-069 revises the final README around the verified AWS–RunPod architecture, summarizes retrieval results and team-confirmed final LLM metrics, and labels HO30 as a public regression evaluation. It preserves team roles, separates local setup into a guide, updates environment examples, and documents the same-filesystem requirement for alternate data directories. PR review also tightened table layout and the evaluation heading and removed the remaining-tasks section. Local links, environment key uniqueness, and diff formatting were checked; runtime tests were not rerun for this documentation change. (commits: 9ad02e3, 5f55e52)
