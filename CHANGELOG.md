...
- PATCH-069 revises the final README around service access, retrieval results and team-confirmed final LLM metrics, and labels HO30 as a public regression evaluation. It preserves team roles, separates local setup into a guide, updates environment examples, and documents the same-filesystem requirement for alternate data directories. PR review tightened table layout and the evaluation heading, removed the remaining-tasks section, and removed an unverified Docker–RunPod deployment diagram and description in favor of the current AWS operations guide. Local links, environment key uniqueness, and diff formatting were checked; runtime tests were not rerun for this documentation change. (commits: 9ad02e3, 5f55e52, 2b837b8)
...
- PATCH-065 keeps My Page room rename and delete controls separate at narrow widths, and makes the month label open a mini calendar. Selecting a date changes the displayed month and marks the selected date. Room deletion still returns to the screen where it started. (commits: 357e970b, 1e1e6a0d, b04cbc6e)
...
- PATCH-063 re-reads handover clause regions in original/corrected photos and stores handover, lease start/end and balance dates separately. Conflicting readings remain unconfirmed; move-in answers use the handover date and request that clause when unreadable. Existing uploads need re-uploading to refresh OCR. (commit: bfb8597)

- PATCH-063 routes registry building questions to document risk/checklist guidance and tracks explicitly stated contract signing status. Document deletion, case switching and ambiguous attachments retain isolation. If generated legal prose fails validation, the chat can still show canonical OCR indicator titles, pages and checks, explicitly labelled as document indicators rather than a verified legal answer. Photographed documents now compare original and corrected/table-layout OCR locally, request clearer views for uncertain values and distinguish unobserved uploaded-page fields from contract omissions. Attachment deletion is now accessible beside the filename without expanding analysis, with the target filename shown before removal. Deleting an attachment retains visible and persisted chat messages; related or untraceable legacy messages are excluded from subsequent model context, new reports and guidance instead of erased. Contract move-in date questions now read dated handover clauses before asking for missing user facts; unclear or conflicting dates request the clause image instead. Clarification questions and reply choices stay in the assistant message; free-text replies in the existing composer are linked to the pending turn. (commit: bfb8597)
