...
구현 커밋 `4b9a929`의 clean 작업 트리에서 재실행한
[문항별 원시 결과](../data/eval/runs/patch006-civil-regression.json)를 저장했다.
보고서의 입력 해시와 검색 설정으로 조건을 확인할 수 있다.
아래 완료 표시는 구현·공개 회귀 검증 완료를 뜻하며 운영 데이터 반영 완료를 뜻하지 않는다.
