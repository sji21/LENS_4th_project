...
- 모델 버전이나 임베딩/인덱스 파이프라인 해시가 다르면 벡터를 재사용하지 않습니다. `dense.py`, `index.py`, 구축 모듈 전체와 `requirements.txt`를 보수적으로 비교하므로 구축 모듈의 로그 변경만으로도 재임베딩할 수 있습니다. 이전 기록에 파이프라인 해시가 없으면 최초 갱신 시 전체 재임베딩합니다. KURE는 검증한 모델 파일을 확인합니다.
...
macOS·RunPod는 **사용자 송지섭이 직접 설치를 확인**했습니다. Mac은 기본 실행 성공 보고이며 세부 검색 수치를 별도로 검증한 결과는 아닙니다. RunPod는 `/opt/lens-venv` 환경에서 KURE 검증·DB 구축·적용·기본 검색 확인까지 완료했고, 일반178·민법26·판례26·안내6청크, 최초 임베딩210+26개가 완료 로그에 기록됐습니다. CUDA 초기화 경고가 있어 GPU 정상 작동은 확인되지 않았으며 설치 속도의 전후 정량 비교·전체 검색 평가·LLM 검증은 별도입니다. 이 기록은 사용자가 제공한 실행 결과이며 Codex가 해당 원격 환경에서 직접 실행한 결과는 아닙니다.
