...
최종 전체 검사: `python -X utf8 -m pytest -q` → **2,726 passed, 3 skipped, 678 subtests passed** (443.68초). 생략은 LangSmith 미설정2건과 로컬 등기 PDF 미지정1건이다. 출력은 `full-tests.log`, 최종 소스·시험·산출물 해시는 `validation.json`에 보존한다. 커밋 전 리뷰 상태이며 메인 작업 트리는 변경하지 않았다.
