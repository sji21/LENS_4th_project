...
리뷰 기준 커밋 `0b202fa`에서 두 P1 오류를 확인했다. OCR로 업로드한 계약서·등본의 분석 문제가 아니라, 검색 질문에 있는 서류·가능 여부 표현을 다른 절차의 요청으로 읽는 오류다.

- `등록민간임대주택에서 전입신고에 필요한 서류를 알려주세요.`: 주택 유형과 일반적인 서류 단어만으로 표준임대차계약서 의도를 활성화했다. 서류 요청을 계약 신고·재계약·계약 체결/작성 또는 명시적인 계약서에 연결하고, 전입신고 첨부서류와 구분한다.
- `전입신고를 하지 않았는데 계약을 해지할 수 있나요?`: 계약 해지의 가능 여부를 전입신고 절차의 요청으로 사용했다. 전입 언급 뒤 연결된 요청 범위에서만 절차 표현을 읽고, 별도의 계약·보증금 주제로 넘어가면 끊는다.

정상적인 계약 서류, 계약서와 전입신고를 함께 묻는 목록, 미완료 전입신고의 직접 질문을 보호한다. 계약 해지·보증금 반환용 서류는 표준계약서 요청으로 승격하지 않는다. 법령 후보·본문 판정·선택기·반환 예산은 변경하지 않았다.

공식 청크의 실제 BM25에서 두 제보 문항의 수정 후 일반 K3가 원래 BM25 상위3건의 근거·순위·점수를 유지함을 확인했다. 전입신고 서류 질문에서는 민간임대47조에 밀렸던 주택임대차보호법3조가 복원됐고, 계약 해지 질문에서는 강제로 넣었던 주민등록16조 대신 원래3위인 주택임대차보호법 시행령4조를 유지한다. 이는 선택기의 잘못된 교체를 막았다는 결과이며, 원래 BM25 근거가 질문 전체에 충분하다는 판정은 아니다. 개발·독립 회귀와 실제 재현 기록은 `data/eval/patch053-review/pr37/`에 보관한다.

고정313입력은 수정 전후의 의도 tuple이 모두 동일했다. 직전 실제 KURE 평가의190개 소스 중 요청 판정 파일만 변경됐고, 이 파일에서도 후보 결합·본문 검증·선택 클래스와 그 설정·현재 질문 추출은 동일하다. 이전 실측 산출물의 manifest도 다시 검증했다. 따라서 이번 P1 수정에서는 KURE를 재실행하지 않고 기존 검색 실측을 보존한다. 이는 새 검색 성능 측정이나 새로운 문형 전체에 대한 무오류 보장이 아니다.

요청 판정 동일성 재확인(모델 호출 없음):

```powershell
python -X utf8 data/eval/patch053-review/pr37/request-gate-audit.py tmp/patch053-pr37-gate-replay.json
python -X utf8 data/eval/patch053-review/pr37/reported-cases.py C:/team_project/patch041-worktree/data/chunks/chunks.jsonl tmp/patch053-pr37-bm25-replay.json
```

이번 리뷰에서 회귀31개를 추가했다. 전체 검사 첫 실행은2,799통과·3생략·678subtests통과였고, 테스트 실행 프로세스의 Git 신뢰 경로 설정 누락으로35실패·7오류가 발생했다. 제품 코드를 바꾸지 않고 해당 프로세스에만 `safe.directory`를 지정한 뒤 실패42개를 `pytest -q --lf --lfnf=none`으로 재실행해42개 모두 통과했다. 두 실행을 합쳐 전체2,841항목이 통과했으며 단일 실행에서 전부 통과한 것으로 기록하지 않는다. 생략은 LangSmith2건·등기 PDF1건이다. 원래 실패 요약·재검사 출력·최종 파일 해시는 `pr37/validation.json`과 같은 디렉터리에 보존한다.

이번 리뷰 수정은 사용자 요청에 따라 구현·테스트·기록을 한 커밋으로 묶는다. 자기 커밋 해시를 기록하기 위한 별도 동기화 커밋은 만들지 않으며, 이 보완의 식별자는 PR #37의 해당 수정 커밋과 파일 해시 기록이다.
