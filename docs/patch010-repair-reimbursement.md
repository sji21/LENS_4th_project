...
PATCH-009 로컬 산출물 `patch009-dev100-20260909`의 입력과 스냅샷 및 PATCH-008 공개 평가 기록이 필요하다. 원본을 보존하고 새 인덱스 복사본을 `tmp/patch010/snapshot`에 만든다. 이미 같은 출력이 있으면 실행이 중단되므로 새 검증 시 출력 경로를 따로 지정하도록 스크립트의 경로를 변경하고 기록한다.

```powershell
.venv/Scripts/python -m pytest -q
.venv/Scripts/python scripts/dev100_diagnostics/repair_regression.py
.venv/Scripts/python scripts/dev100_diagnostics/repair_public_regression.py
```
