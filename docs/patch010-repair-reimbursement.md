...
PATCH-021 이후 위 공개 회귀 도구는 채널 분리 진단 형식으로 변경됐다. 예전 혼합 TOP5와의 점수 동일성을 강제하지 않으며 settings/results 및 문항별 필수 근거 증감을 출력한다. 당시 PATCH-010의 원래 동일성 검증을 재현하려면 해당 커밋을 체크아웃한다. 최신 동작은 [PATCH-021 기록](patch021-civil-result-channel.md)을 따른다.
