...
검색·평가 관련 테스트 87개와 subtest 30개가 통과했다.
기존 로컬 평가셋을 이용한 실행에서 법령 Holdout Hit@3 17/18,
판례 Dev Hit@2 12/13, 판례 외부 표현 Hit@2 7/8을 확인했다.
판례 Dev 입력은 3차 로컬 최종 보고서의 질문·정답 13행을 그대로 추출했고,
외부 표현 입력은 3차 로컬 사본을 사용했다. 두 입력 모두 커밋하지 않는다.

법령 Dev 첫 CLI 실행은 Hit@3 23/24, Recall@3 93.75%로 기존 보고서와 달랐다.
같은 데이터의 기존 `compare_law_top3` 실행은 Hit@3·Recall@3 100%를 재현했고,
추가로 동일 프로세스에서 반환량 5/10만 바꾼 두 질문의 상위 순위는 같았다.
따라서 차이를 반환량 때문이라고 단정할 수 없다. 검색·모델 설정을 튜닝하거나
실패 질문을 제외하지 않았으며, 실행 간 차이의 원인은 별도 진단이 필요하다.
PATCH-003은 측정 코드 저장소화 완료이며, 법령 Dev의 모든 실행이 동일하다는
보증이나 검색 개선 완료를 뜻하지 않는다. 이후 성능 비교 시 이 차이를 함께 확인한다.
