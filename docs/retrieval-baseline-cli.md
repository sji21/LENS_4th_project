...
검색 설정은 실행에 실제로 쓰인 검색기 객체에서 읽어 자료 유형별로 기록한다. RRF `k`,
후보 깊이, BM25 `k1`·`b`·문자 n-gram 이 여기 들어간다. 값을 코드에 복사해 두면 검색을
다시 튜닝했을 때 보고서만 옛 값을 계속 적게 되므로 복사본을 두지 않는다. 자료 유형마다
설정이 다를 수 있어 하나로 합치지도 않는다 — 현재 법령은 RRF `k=5`, 판례·안내는 `k=60`이다.
...
구현 커밋 `fbc946c` 와 리뷰 반영 커밋 기준이다. 설정 기록 방식을 실측값 읽기로 바꾼 뒤
다시 측정했고, 지표와 테스트 결과는 모두 같았다. 아래 수치는 재현되는 실행 기준이며,
재현되지 않는 관측은 이 절 끝에 따로 남긴다.
...
이전 실행에서 법령 Dev Hit@3 23/24 를 관측했으나 현재 커밋에서는 재현되지 않았고,
**원인은 확인되지 않았다.** 어느 시점의 코드에서 나온 값인지도 특정하지 못했다.
3차 저장소에도 같은 수치나 그에 해당하는 기록은 없다. 다만 3차
`docs/eval-audit.md` 는 법령 Dev 11문항의 정답 라벨을 재검토 대상으로 올려두고
판정을 비워 둔 상태이므로, 법령 Dev 는 라벨에 민감할 수 있다는 배경만 확인된다.
이를 원인으로 단정하지 않는다. 이후 성능 비교에서 같은 차이가 다시 보이면 진단한다.
