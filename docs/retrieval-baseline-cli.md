...
**원인은 확정하지 못했다.** 당시 결과는 로컬 `tmp/patch003-law-dev-check.json` 에
남아 있어 다음까지는 확인된다.

- 실행 시점 `2026-09-08T00:16Z`, 기록된 HEAD 는 `0e95f04`
- 입력 4개(법령·판례·안내 청크, `dev.jsonl`)의 SHA-256 과 인덱스 검증 165건이
  현재 실행과 **모두 같다.** 데이터는 차이의 원인이 아니다
- 차이는 두 문항에 국한된다. `dev-011` 은 정답 순위 4위에서 1위로, `dev-001` 은
  4위에서 3위로 올라갔다. Hit@3 을 가른 것은 `dev-011` 이다

확정하지 못한 것은 **실행 당시의 정확한 코드 상태**다. 그 시점에는 이 패치의 작업이
모두 미커밋이었으므로 기록된 HEAD 는 실제로 돌아간 코드를 설명하지 못한다. 당시
결과에 `worktree_dirty` 필드가 없는 것으로 보아 구현 커밋보다 앞선 코드인 것은
분명하지만, 그 코드는 커밋되지 않아 복원할 수 없다.

3차 저장소에도 같은 수치나 그에 해당하는 기록은 없다. 3차 `docs/eval-audit.md` 가
법령 Dev 11문항의 정답 라벨을 재검토 대상으로 두고 판정을 비워 둔 상태라 법령 Dev 가
라벨에 민감할 수 있다는 배경은 있으나, 이를 원인으로 단정하지 않는다. 해당 감사
정리는 `PATCH-005` 로 등록했다. 이후 성능 비교에서 같은 차이가 다시 보이면 진단한다.
