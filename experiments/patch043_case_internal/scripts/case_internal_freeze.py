...
def main():
    ...
    tracked=[name for name in subprocess.check_output(['git','ls-files','-z'],cwd=repo).decode('utf-8').split('\0')
             if name and (repo/name).is_file()]
    added=[str(p.relative_to(repo)) for p in experiment_root.rglob('*.py')]
