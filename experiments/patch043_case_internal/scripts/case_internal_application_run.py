...
def main():
    ...
    write(args.output/'INPUT_SEAL.json',{'profile_sha256':sha(args.profile),'jobs_sha256':sha(args.jobs),
        ...
        'code':({str(p.relative_to(project_root)):sha(p) for p in (project_root/'src/retrieval').glob('*.py')}
                | {str(p.relative_to(project_root)):sha(p) for p in experiment_root.glob('*.py')}),
