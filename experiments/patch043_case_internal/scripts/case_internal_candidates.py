...
def main():
    ...
    write(args.output/'INPUT_SEAL.json',{'questions_sha256':sha(args.questions),'chunks_sha256':sha(args.data/'chunks/cases.jsonl'),
        ...
        'retrieval_code':{'experiments/patch043_case_internal/retrieval.py':sha(Path(__file__).resolve().parents[1]/'retrieval.py')},
    ...
    if args.baseline_data:
        from experiments.patch043_case_internal.scripts.case_internal_run import baseline
        from experiments.patch043_case_internal.scripts.case_internal_prepare_run import capture
