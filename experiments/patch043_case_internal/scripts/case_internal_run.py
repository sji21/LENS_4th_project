...
def main():
    ...
    write(args.output/'INPUT_SEAL.json', {
        ...
        'case_code': {
            'experiments/patch043_case_internal/retrieval.py': sha(Path(__file__).resolve().parents[1]/'retrieval.py'),
            'experiments/patch043_case_internal/profile.py': sha(Path(__file__).resolve().parents[1]/'profile.py'),
        }})
    try:
        import torch
        import psutil
