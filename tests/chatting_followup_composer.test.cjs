...
test('cancelling a follow-up preserves the draft and ignores the aborted reply', async () => {
  const handlers = {};
  const rendered = [];
  let operation;
  const button = {addEventListener: (_, fn) => handlers.cancel = fn};
  const context = {
    AbortController, activeChat: null, busy: true, externalBusy: false,
    controls(){}, notice(){}, async syncState(){},
    input:{value:'두 달 남았어',focus(){}}, replyingTo:null, conversationId:'chat',lastRendered:'',
    form:{dataset:{sendUrl:'/send',cancelUrl:'/cancel',replyTo:'pending-1'},addEventListener:(_,fn)=>handlers.submit=fn},
    action:(_,fn)=>operation=fn(), $:id=>id==='cancel-answer'?button:{value:'',append(){}},
    renderMessage:()=>({}),scrollBottom(){},render:data=>rendered.push(data),
    submission:{get:()=> 'request-1',clear(){}},
    post:async (url,payload,id,options)=> {
      assert.equal(id,'request-1');
      if(url==='/cancel') return {cancelled:true};
      assert.equal(payload.reply_to,'pending-1');
      return new Promise((resolve,reject)=>options.signal.addEventListener('abort',()=>reject(new Error('aborted'))));
    },
  };
  const code=source.slice(source.indexOf('  form.addEventListener("submit"'),source.indexOf('  const reportForm'));
  vm.runInNewContext(code,context);
  handlers.submit({preventDefault(){}});
  assert.equal(context.input.value,'');
  await handlers.cancel(); await operation;
  assert.equal(context.input.value,'두 달 남았어');
  assert.equal(context.form.dataset.replyTo,'pending-1');
  assert.equal(context.activeChat,null);
  assert.equal(context.busy,false);
  assert.equal(rendered.length,1);
  assert.equal(rendered[0].cancelled,true);
});
